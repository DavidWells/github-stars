...
function createStarTable(
  sortedByStarredDate, 
  datePadding = true,
  maxWidth = MAX_TABLE_CONTENT_WIDTH
) {
  /* Make HTML Table */
  let html = `<table>
  <tr>
  ...
  const topicsRender = (topics && topics.length > 0) ? `<br/>${stringUtils.stringBreak(tinyText(`Tags: ${topics.map((topic) => `#${topic}`).join(' ')}`), maxWidth + 60).join('<br/>')}` : ''
  ...
  const BlankLine = (datePadding) ? BRAILE_SPACE.repeat(10) + '<br/>' : ''
  const localReadMe = localReadMePath(repo)
  
  html += `
  <tr>
  <td><a href="${url}">${stringUtils.stringBreak(repo, maxWidth).join('<br/>')}</a>${inlineMeta}${topicsRender}${_description}</td>
